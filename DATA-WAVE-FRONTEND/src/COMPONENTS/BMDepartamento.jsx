...
function AdminEditDepartamento(props) {
  ...
  const loadDepartamento = (isSearch = false) => {
    if (!isSearch) {
      axios.get("http://localhost:3001/departamento/lista")
        .then((response) => {
      ...
      const filtered = departamento.filter((departamento) =>
  ...
  const editDepartamento = (departamentoId) => {
    const departamentoToEdit = departamento.find((departamento) => departamento.id === departamentoId);
  ...
  const confirmEdit = () => {
    axios
      .put(`http://localhost:3001/departamento/actualizar/${selectedDepartamento.id}`, {
    ...
    console.log("Departamento editado", "El Departamento ha sido editado correctamente", "success");
  }
  ...
  const deleteDepartamento = (departamentoId) => {
    axios
      ...
      .then((response) => {
        ...
        setSelectedDepartamento(null);
        setNewDepartamentoName("");
        console.log("Departamento eliminado", "El departamento ha sido eliminado correctamente", "success");
      })
      .catch((error) => {
        ...
        {filteredDepartamento.map((departamento) => (
          <div className="flex" key={departamento.id}>
            ...
            <button className="border-2 rounded-lg border-gray-300 h-10 w-20 titulo_tarjetas m-4">
              <Link to="/departamento/actualizar">GESTIONAR CATEGORIA</Link>
            </button>
            <button className="border-2 rounded-lg border-gray-300 h-10 w-20 titulo_tarjetas m-4">
